...
.controller('JobCtrl', function ($scope, $rootScope, coreService, authoritiesService, alertFactory, dialogs, $filter, $state, $timeout) {
    ...
    $scope.gridInfo = {
        ...
        cols: [
             { name: 'RowIndex', heading: 'RowIndex', isHidden: true },
              ...
              { name: 'Week', heading: 'Tuần', className: 'text-center pd-0 break-word' },
              { name: 'Month', heading: 'Tháng', className: 'text-center pd-0 break-word' },
              { name: 'Year', heading: 'Năm', className: 'text-center pd-0 break-word' },
              { name: 'Description', heading: 'Báo cáo công việc', className: 'text-center pd-0 break-word' },
        ...
        onActionClick: function (row, act) {
            ...
            switch (act) {
                case 'view':
                    //                    console.log('row', row);
                    $state.transitionTo('editjob', { jobId: row.ID || row });
    ...
    $scope.yearSelectList = yearSelectList;
    $scope.monthSelectList = monthSelectList;
    $scope.weekSelectList = weekSelectList;
    ...
    $scope.$watch('jobId', function (newVal, oldVal) {
        if (typeof newVal != 'undefined') {
            $rootScope.showModal = true;
            coreService.getListEx({ JobId: $scope.JobId, Sys_ViewID: 22 }, function (data) {
                //console.log('CustomerID', data);
                convertStringtoNumber(data[1], 'YearId');
                convertStringtoNumber(data[1], 'MonthId');
                convertStringtoNumber(data[1], 'WeekId');
    ...
    $scope.actionEntry = function (act) {
        if (typeof act != 'undefined') {
            ...
            entry.Action = act;
            entry.Sys_ViewID = 22; //$scope.gridInfo.sysViewID;
