...
var treeViewData = new Object();
treeViewData.images = treeViewData_imageArray;

(function () {
    for (var i = 0; i < 19; i++) {
        var preLoad = new Image();
        if (treeViewData_imageArray[i].length > 0)
            preLoad.src = treeViewData_imageArray[i];
    }
})();
treeViewData.lastIndex = 9;
treeViewData.name = 'treeViewData';

function TreeView_ToggleNode(data, index, nodeName, lineType, childrenName) {
    if (!data) {
        return;
    }
